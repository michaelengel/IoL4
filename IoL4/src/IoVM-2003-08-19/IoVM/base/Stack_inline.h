...
{
  self->top ++;
  if (self->top == self->memEnd) Stack_resize(self);
  *(self->top) = item;
}
...
{
  void *top = *(self->top);
  if (self->items != self->top) self->top --;
  return top;
} 
...
IOINLINE void Stack_popMark(Stack *self)
{
  ...
  self->top --;
  if (self->lastMark) self->lastMark = (ptrdiff_t)(self->items[self->lastMark]);
}

IOINLINE void *Stack_top(Stack *self) { return *(self->top); }
